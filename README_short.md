...
`fgvlm.visual_attributes`는 이미지와 segmentation mask를 입력으로 받아, 객체의 시각적 속성을 구조화된 object 중심 attribute 형태로 추출하는 모듈입니다.
...
### 3-4. attribute dict 생성

마지막으로 아래 형식의 object 중심 attribute dict를 만듭니다.
...
여기서 `"lesion"`은 예시 이름입니다. 실제 사용 시에는 `tumor`, `cyst`, `mass`, `nodule`처럼 데이터에 맞는 객체 이름을 직접 넣어 사용하는 것을 권장합니다.
...
`measurements`는 분석과 디버깅에 유용하고, `attributes`는 후속 captioning이나 structured dataset 생성에 바로 사용할 수 있습니다.
...
- `fine-grained-vlm/tests/test_visual_attributes_batch.py`
...
## 12. 폴더 단위로 바로 실행하는 방법

이미지들이 한 폴더에 있고, 마스크들이 다른 폴더에 있을 때는 배치 실행 코드를 바로 사용할 수 있습니다.

### 준비 형태 예시

아래처럼 데이터를 준비했다고 가정하겠습니다.
...
이 구조에서 다음처럼 이해하면 됩니다.

- `images/`에는 원본 이미지 파일을 넣습니다.
- `masks/`에는 segmentation mask 파일을 넣습니다.
- 객체 이름은 더 이상 `lesion`으로 고정하지 않습니다.
- 단일 객체 데이터셋이면 사용자가 직접 객체 이름 하나를 넣습니다.
- 여러 객체 데이터셋이면 사용자가 `CSV` 파일에서 각 mask와 객체 이름을 직접 연결합니다.

### 가장 쉬운 실행 방법

다음 스크립트를 사용하면 됩니다.

- [run_visual_attributes.sh](/home/bongdong2/bongdong2_workspace/fine-grained-vlm/scripts/run_visual_attributes.sh:1)
- [run_visual_attributes_split_components.sh](/home/bongdong2/bongdong2_workspace/fine-grained-vlm/scripts/run_visual_attributes_split_components.sh:1)

### 경우 A. 이미지마다 객체 종류가 하나인 경우

예를 들어 모든 mask가 `tumor`를 의미한다면, 객체 이름을 직접 넘겨서 실행합니다.
...
이 경우 4번째 인자인 `tumor`가 모든 mask에 공통으로 적용됩니다.

### 경우 B. 이미지마다 객체 종류가 여러 개인 경우

이 경우에는 mask와 object를 직접 연결하는 `CSV` 파일을 사용합니다.
...
case002.png,case002_calcification.png,calcification
```
...
스크립트는 4번째 인자가 파일이면 `CSV 매핑 파일`로 보고, 파일이 아니면 `단일 객체 이름`으로 처리합니다.
...
- `summary.json`
  - 전체 처리 개수, skip된 이미지 수, size threshold 등 요약 정보
- `all_results.json`
  - 전체 이미지 결과를 한 파일에 모아 둔 통합 결과
- `per_image/case001.json`
  - 이미지 한 장에 대한 객체 속성 결과

즉, 공개용으로 설명하면 아래처럼 안내하면 됩니다.

1. 이미지 파일을 `/data/my_dataset/images`에 저장합니다.
2. 마스크 파일을 `/data/my_dataset/masks`에 저장합니다.
3. 객체가 하나면 객체 이름을 하나 정합니다. 예: `tumor`
4. 객체가 여러 개면 `image_filename, mask_filename, object_name` 형식의 CSV를 만듭니다.
5. 아래 명령 중 하나를 실행합니다.
...
6. 그러면 `/data/my_dataset/visual_attributes_output` 아래에 `summary.json`, `all_results.json`, `per_image/*.json`이 생성됩니다.

### Python 모듈로 직접 실행하는 방법

쉘 스크립트 대신 Python 모듈로 바로 실행할 수도 있습니다.
...
PYTHONPATH=src python -m fgvlm.visual_attributes \
  ...
  --output-dir /data/my_dataset/visual_attributes_output \
  --object-name tumor
```
...
- `--calibration-masks-dir`
  - size threshold를 맞출 때 사용할 마스크 폴더를 따로 지정할 수 있습니다.
  - 지정하지 않으면 `--masks-dir`를 그대로 사용합니다.
- `--object-name tumor`
  - 모든 mask가 같은 객체를 의미할 때 사용합니다.
- `--object-map-csv /path/to/object_map.csv`
  - 여러 객체가 섞여 있을 때, mask와 object 이름을 명시적으로 연결합니다.
- `--split-components`
  - 하나의 mask 파일 안에 떨어진 영역이 여러 개 있을 때, 이를 분리해 각각 다른 객체 인스턴스로 저장합니다.
- `--infer-object-name`
  - `case001_tumor.png`처럼 mask 파일명에 객체 이름이 들어 있을 때 자동 추론합니다.
  - 다만 가장 안전한 방법은 `--object-name` 또는 `--object-map-csv`를 직접 넣는 것입니다.

예를 들어 disconnected object를 분리하고 싶다면 이렇게 실행하면 됩니다.
...
### Python 코드 안에서 직접 호출하는 방법

```python
from fgvlm.visual_attributes import run_folder_extraction

summary = run_folder_extraction(
    images_dir="/data/my_dataset/images",
    masks_dir="/data/my_dataset/masks",
    output_dir="/data/my_dataset/visual_attributes_output",
    object_name="tumor",
)
```

실행이 끝나면 `summary`에는 처리된 이미지 수, 객체 수, 결과 파일 경로 등이 들어 있습니다.

## 13. 한 줄 요약

이 모듈은 "이미지 + mask"에서 객체의 위치, 크기, 모양, 방향, 경계 특성을 자동으로 계산하고, 이를 후속 파이프라인에서 바로 쓸 수 있는 구조화된 attribute dict 형태로 정리해 주는 범용 도구입니다.
